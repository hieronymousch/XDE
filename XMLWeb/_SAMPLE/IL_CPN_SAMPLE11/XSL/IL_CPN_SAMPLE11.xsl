...
<xsl:variable name="vSampleNBR" select="'11'" />
		...
		<body>
			...
			<xsl:if test="$vMODE='Main'">
				...
				<xsl:call-template name="Generic_Table_DBWEB">
					...
					<!-- Appel d'un Template générique : ici le XTAB -->
			...
			<div>
				...
				<!--<xsl:if test="$vMODE='Drill-Down'">-->
					...
					<xsl:call-template name="Generic_Table_DBWEB">
						...
						<!-- Ici l'URL de Drill-Down 2 (boule verte/rouge) est spécifiée (le pMODE et pLANG ne doivent pas être spécifié) -->
...
<xsl:template match="row">
	...
	<xsl:choose>
		<xsl:when test="$Detail_Data='SPE_01_XTAB_ASS'">
			...
			<xsl:variable name="vXMLFileName">IL_CPN_SAMPLE11.xml</xsl:variable>
			...
			<xsl:call-template name="DynamicXTable">
				...
				<xsl:with-param name="vInfoR1" select="'AddInfo'"/>
				...
				<!--xsl:with-param name="vAddInfo1" select="'TEST'"/--> 
		...
		<xsl:when test="$Detail_Data='DEMO'">
			<xsl:value-of select="round(number($tPD2) div 77*100)"/>
		</xsl:when>
		<xsl:otherwise>
