	...
	<meta name="author" content="Ibanez Crescencio"/>
		...
		<body>
			...
			<center>
				<h1>
					<xsl:call-template name="FromDic2Rep">
						<xsl:with-param name="Title2S" select="'REPORT03_TITLE'"/>
					</xsl:call-template>
				</h1>
			</center>
			...
			<xsl:call-template name="Generic_Table_DBWEB">
				...
				<xsl:with-param name="Col_Hidden" select="'7,11'"/>
				<xsl:with-param name="Col_Sorting" select="'1-desc'"/>		
			</xsl:call-template>
...
<!-- SPE Specific -->
<!--
<xsl:template match="@FUNCTION" mode="genericTableCel">                  
    <td>
	    <button class="btn btn-primary" type="button">
				Messages <span class="badge"><xsl:value-of select="../@ASSET"/></span>
		</button>
    </td>
</xsl:template>
-->
<!-- SPE 'Classic' but called outside SQL : Replace concat('MRN_SPE09#06##',ASSET,'#1#',ASSET) ASSET  by ASSET-->
<!--
<xsl:template match="@ASSET" mode="genericTableCel">             
    <td>
        <xsl:call-template name="SPE_Feature">                            
                        <xsl:with-param name="FieldVal" select="concat('MRN_SPE09#06##',.,'#1#',ASSET)"/>
        </xsl:call-template>
    </td>
</xsl:template>
-->
<xsl:template match="row">
	...
	<xsl:param name="tPD4"/>
	<xsl:choose>
		<xsl:when test="$Detail_Data='TESTDEMOSPE22'">
			<button class="btn btn-primary" type="button">
  				Messages <span class="badge"><xsl:value-of select="$tPD1"/></span>
			</button>
		</xsl:when>
		<xsl:otherwise>
