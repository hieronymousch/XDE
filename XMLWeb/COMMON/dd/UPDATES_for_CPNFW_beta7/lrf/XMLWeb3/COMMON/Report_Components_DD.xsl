...
<!-- Active Templates used for CPN -->
			...
			<xsl:when test="$dT_Type='99'"> 	<!-- For Std Table Result, all features activated (Version to Copy) -->
				<script>
								...
								buttons: [
									{
										text: 'Red',
										action: function() {
											heatMap(243,32,117,'#dT_<xsl:value-of select="$Table_Name"/>');
										}
									},
									{
										text: 'Green',
										action: function() {
											heatMap(118,246,68,'#dT_<xsl:value-of select="$Table_Name"/>');
										}
									},
									{
										text: 'Blue',
										action: function() {
											heatMap(52,119,220,'#dT_<xsl:value-of select="$Table_Name"/>');
										}
									},
									{
										text: 'Orange',
										action: function() {
											heatMap(255,179,71,'#dT_<xsl:value-of select="$Table_Name"/>');
										}
									},
									{
										text: 'Grey',
										action: function() {
											heatMap(196,195,208,'#dT_<xsl:value-of select="$Table_Name"/>');
										}
									}
								]
					        }
					...
					$('#dT_<xsl:value-of select="$Table_Name"/>').on('column-visibility.dt', function(e,settings,column,state) {
						...
						var tblCols = $('.dt-button-collection a[aria-controls='+dtID+']').length;
	...
	<xsl:template name="DynamicXTable"> 			<!-- vX - Allow generation of Dynamic Crosstable and DD on Detail  -->
		...
		<xsl:param name="Lang" select="'EN'"/>
		<xsl:param name="RepDic" select="'N'"/>				<!-- Use of Report Dictionnary. Default 'N' / 'Y' -->
		<xsl:param name="dT_HeatMap" select="'N'"/>			<!-- Show heatmap button. Default 'N' / 'Y' -->
