...
<!-- Common Pillar Node FW - Components - By CPN Dev Core Team
	 ...
	 3.0.0 (Beta Do Not Use For Prod)
	 	# Report_Components.xsl
	 		- All COMMON Templates (are moved to) 
		 	@ DynamicXTable
		 		- Added "Records=0" parameter when call @Node_Std_Table for automatic calculation of Records in XTable
		 		- Started (XTable HeatMap) - Not Finished. Must be Vertcial/Horizontal/Global and called by User
		 	@ Node_Std_Table
		 		- For dT_Type=99 and when Records=0, Nbr of line will be calculated for Paging (less 25 no paging, else Yes)
		 	@ Node_Std_Head 
		 		- CPN introduced has based Config
		 	@ Generic_Table_DBWeb
		 		- Alphabet is depreacated - DO NOT USE ANYMORE (Warning and Feature will be deleted in version 1.1)
-->

<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
<xsl:output method="html" indent="yes"/>

<!-- Active Templates used for FW3 -->
	...
	<xsl:template name="SPE_Feature">				<!-- Template For Special Features MR Node -->
			...
			<xsl:when test="substring($FieldVal,1,7)='MRN_SPE'">
				...
				<xsl:choose>
					...
					<xsl:when test="$SPE_Type='05'"> <!-- Drill Down -->
						...
						<xsl:variable name="DDCriteria">
							<xsl:value-of select="substring-after($SPE_Val,'#')"/>
						</xsl:variable>
						<xsl:variable name="DDidTD">
							<xsl:value-of select="substring-before($SPE_Val,'#')"/>
						</xsl:variable>
					...
					<xsl:when test="$SPE_Type='08'"> <!-- Hide Cell Content -->
						...
						<xsl:element name="font">
							<xsl:attribute name="color"><xsl:value-of select="$MRN_Param"/></xsl:attribute>			
							<xsl:value-of select="$ValToShow"/>
						</xsl:element>
					</xsl:when>
	...
	<xsl:template name="Checks"> 				<!-- Template with All Checks -->
		<!-- Debug Message Nr -->
		...
		<script>			
			if(localStorage.getItem('<xsl:value-of select="$vNodeConfig"/>_Settings')===null) {
				...
				if('<xsl:value-of select="distinct-values(upper-case(//dbquery[1]/descriptor/parameters/param[@name='pCDN']/@value))"/>'=='') {
		...
		<!-- pLANG not correct -->
			...
			<script>
				if (document.URL.indexOf("pLANG=FR")==-1 &amp;&amp; document.URL.indexOf("pLANG=NL")==-1 &amp;&amp; document.URL.indexOf("pLANG=EN")==-1) {
					LangMsg='<center><h1>Incorrect Language parameter <small>(pLANG)</small></h1><h2>in your URL</h2></center>Parameter pLANG <u><b>MUST</b></u> be present in your URL and only 3 values are allowed : FR, NL or EN.<br/>Incorrect use of pLANG can cause bad report rendering (Layout).<br/>Possible Cause :<ul><li>Parameter Language not defined in the <a href="/" target="_blank">Node Homepage</a>. Go on Homepage and select language on the Upper Right Part of Homepage</li><li>Bad Link. Modify link or communicate bad link to Report Owner. For MR Node contact per EMail <xsl:value-of select="$vWebMaster"/></li></ul>You can also modify link manually<br/>'
					MagPopUp(LangMsg);
				};
			</script>
		...
		<script>
			if (document.URL.indexOf("mrnode")==-1 &amp;&amp; document.URL.indexOf("opsnode")==-1 &amp;&amp; document.URL.indexOf("qrnode")==-1 &amp;&amp; document.URL.indexOf("localhost")==-1) {
				MRNNotif('3','T19 - Using a Local Dev Node ... Be Aware','You are requesting a report on a Local Node. No garanties are provided on data validity',10,"ALL");
			};
		</script>
		...
		<xsl:if test="$vMODE='Main' or not($vMODE) or $vMODE=''">
			...
			<!-- URL Not match with RepId Notification only for Main Report
			<xsl:variable name="vURL" select="distinct-values(//dbquery[@id='Report_Info']/rows/row/@dbn_rep_url)"/>
			<script>
				CheckURL('<xsl:value-of select="$vURL"/>','<xsl:value-of select="$vUSER"/>');
			</script>
			-->
		</xsl:if>
	...
	<xsl:template name="Recurse_Image">
	    <xsl:param name="num" />
	    <xsl:param name="text" />
	...
	<xsl:template name="Recurse_Line">
	...
	<xsl:template name="Node_Std_Head"> 			<!-- Template For Std loading of JS/CSS in Report Head -->
	...
	<xsl:template name="Generic_Table_DBWEB"> 		<!-- Template For Automatic Generation Frame and table based on DBWEB -->
		<xsl:param name="DBWEB_Name" select="'TO_BE_FILLED'"/>	<!-- MANDATORY 		DBWeb Query Name -->
		...
		<xsl:param name="UdTN" select="'NADA'"/>				<!-- Unique dT Name (usefull if use several times same DBWEB -->
		<xsl:param name="RepDic" select="'Y'"/>					<!-- Use of Report Dictionnary. Default 'Y' / 'N' (Filtering problem when using N) -->
		<xsl:param name="Lang" select="'EN'"/>					<!-- Language. Default 'EN'/'FR'/'NL' -->
		...
		<xsl:variable name="RecFound"><xsl:value-of select="count(//dbquery[@id=$DBWEB_Name]/rows/row)"/></xsl:variable>
		...
		<xsl:choose>
			<xsl:when test="$Show_Empty='N' and $RecFound='0' and count(//dbquery[@id=$DBWEB_Name]/error)=0">
				<!-- Nada because no data -->
			</xsl:when>
			<xsl:otherwise>
				<!-- Show Content (even no records) -->	
				<xsl:choose>
					<xsl:when test="substring($Frame,1,1)='Y'">
						...
						<!-- Show Frame means Table Embedded -->
						<!-- all embedded in a Table because colspan bug in IE10. Now Layout is OK for Firefox -->
										...
										<xsl:choose>
											...
											<xsl:when test="count(//dbquery[@id=$DBWEB_Name]/error)=1">
												...
												<center>
													...
													<br/>
													Please Report this Error to MR-Mgt (Cdt Ibanez or Adjt Dubus)</b><br/>
												</center>
											</xsl:when>
											<!-- DBWeb Not in Error -->
											<xsl:otherwise>
												...
												<xsl:choose>
													<xsl:when test="$Detail_Data='Generic'">
														...
														<xsl:call-template name="Node_Std_Table">
															...
															<xsl:with-param name="dT_Col_Order" select="'Y'"/>	
															<xsl:with-param name="dT_Filter" select="$dT_Col_Order"/>
	...
	<xsl:template name="Generic_Detailed_Table"> 	<!-- Template For Automatic Generation of Detailed table -->	
		...
		<table cellpadding="0" cellspacing="0" border="0" width="100%">
						...
						<xsl:for-each select = "(//dbquery[@id=$DBWEB_Name])[1]/columns/column">
							<th>
								<xsl:choose>
									...
									<xsl:otherwise>
										CP_<xsl:call-template name="FromDic2Rep">
											<xsl:with-param name="Title2S" select="@name"/>
											<xsl:with-param name="Lang" select="$Lang"/>
											<xsl:with-param name="dictionary" select="$dictionary"/>
											<xsl:with-param name="tDDConfig" select="'CP'"/>
										</xsl:call-template>
									</xsl:otherwise>
								</xsl:choose>
							</th>
						...
						<xsl:for-each select = "(//dbquery[@id=$DBWEB_Name])[1]/columns/column">
							<th>
								<xsl:choose>
									...
									<xsl:otherwise>
										IP_<xsl:call-template name="FromDic2Rep">
											<xsl:with-param name="Title2S" select="@name"/>
											<xsl:with-param name="Lang" select="$Lang"/>
											<xsl:with-param name="dictionary" select="$dictionary"/>
						...
						<xsl:for-each select = "(//dbquery[@id=$DBWEB_Name])[1]/columns/column">
							<th>
								<xsl:choose>
									...
									<xsl:otherwise>
										CIP_<xsl:call-template name="FromDic2Rep">
											<xsl:with-param name="Title2S" select="@name"/>
											<xsl:with-param name="Lang" select="$Lang"/>
											<xsl:with-param name="dictionary" select="$dictionary"/>
											<xsl:with-param name="tDDConfig" select="'CIP'"/>
										</xsl:call-template>
									</xsl:otherwise>
								</xsl:choose>
							</th>
						...
						<xsl:for-each select = "(//dbquery[@id=$DBWEB_Name])[1]/columns/column">
							<th>
								<xsl:choose>
									...
									<xsl:otherwise>
										C_<xsl:call-template name="FromDic2Rep">
											<xsl:with-param name="Title2S" select="@name"/>
											<xsl:with-param name="Lang" select="$Lang"/>
											<xsl:with-param name="dictionary" select="$dictionary"/>
											<xsl:with-param name="tDDConfig" select="'C'"/>
						...
						<xsl:for-each select = "(//dbquery[@id=$DBWEB_Name])[1]/columns/column">
							<th>
								<xsl:choose>
									...
									<xsl:otherwise>
										X_<xsl:call-template name="FromDic2Rep">
											<xsl:with-param name="Title2S" select="@name"/>
											<xsl:with-param name="Lang" select="$Lang"/>
											<xsl:with-param name="dictionary" select="$dictionary"/>
											<xsl:with-param name="tDDConfig" select="'X'"/>
										</xsl:call-template>
									</xsl:otherwise>
								</xsl:choose>
							</th>
			...
			<tbody>
				<xsl:for-each select = "//dbquery[@id=$DBWEB_Name]/rows/row">
					<tr>
						<xsl:for-each select="@*">
							<xsl:variable name="RowNbr"><xsl:value-of select="position()"/></xsl:variable>
							<td align='center'>
								<xsl:call-template name="SPE_Feature">
			...
			<xsl:if test="count(//dbquery[@id=$DBWEB_Name]/rows/row)>=20 or $ForceFooter='Y'">
				<tfoot>			
					<tr>
	...
	<xsl:template name="Node_Std_Table">			<!-- Template For Datatable Configuration -->
				...
				<script>
					...
					<xsl:if test="$Col_Filtering!='None'">
						...
						$('#dT_<xsl:value-of select="$Table_Name"/> tfoot th.CPN_Search').each( function () {
					        if(keyValue2===null){
					        	var Filtervalue = '';
					        	$(this).append( ' <input type="text" size="1" value="'+ Filtervalue +'" placeholder="?" />' );
					        	} else {
					        	...
					        	if (Filtervalue==='') {
					        		...
					        		$(this).append( ' <input type="text" size="1" style="background-color:#f1c40f" value="'+ Filtervalue +'" placeholder="?" />' );
					        		}
					        	};
					    } );
					</xsl:if>
					...
					var oTable_dT_<xsl:value-of select="$Table_Name"/> = $('#dT_<xsl:value-of select="$Table_Name"/>').DataTable( {
						dom: 'lB<xsl:if test="$dT_Filter='Y'">f</xsl:if><xsl:if test="$dT_Paging='Y' or ($dT_Paging='N' and $Records=0)">p</xsl:if>AR&lt;"clear">Cirt',
						<xsl:choose>
						...
						"buttons": [
					        ...
					        <xsl:if test="$dT_XLS='Y' or $dT_PDF='Y'">{
					            extend: 'collection',
					            text: 'Export',
					            buttons: [ <xsl:if test="$dT_XLS='Y'">'excelHtml5',</xsl:if><xsl:if test="$dT_PDF='Y'">'pdfFlash',</xsl:if> ]
					        },</xsl:if>
							{
					            extend: 'collection',
					            text: 'Tools',
					            buttons: [ 
                					...
                					},
                					{
										...
										action: function() {
											...
											MRNNotif('1','T06 - Tables Result Personnalization Cleared','State Cleared (Column Filters, Visibility and Ordering). Reset will take effect on next Refresh or Query for the table contained in this block',10,'ALL');
											}
                					},
					...
					<xsl:if test="$Col_Filtering!='None'">
					    ...
					    oTable_dT_<xsl:value-of select="$Table_Name"/>.columns().eq( 0 ).each( function ( colIdx ) {
					        $( 'input', oTable_dT_<xsl:value-of select="$Table_Name"/>.column( colIdx ).footer() ).on( 'keyup change', function () {
	...
	<xsl:template name="DynamicXTable"> 			<!-- vX - Allow generation of Dynamic Crosstable and DD on Detail  -->
		...
		<xsl:param name="vR1" select="'YEAR'"/> 			<!-- First Horizontal Grouping -->
		<xsl:param name="vR2" select="'STATUS'"/> 			<!-- Second Horizontal Grouping. If value is equal to vR1 only one column will be displayed -->
		<xsl:param name="vC1" select="'PRIORITY'"/> 		<!-- Vertical Grouping. One column per value found -->
		...
		<!-- Prototype Colorization XTab 
		<script>
			alert($("#dT_XTAB_SUM_0006_10EA td.Mosaic").length);
			var ValMax=0;
			$("#dT_XTAB_SUM_0006_10EA td.Mosaic a").each(function() {
				alert($(this).html()+' is greater than'+ValMax);
				if($(this).html()*1>ValMax*1) {
					alert('Greater')
					ValMax=$(this).html()
					} else {
					alert('smaller')
	...
	<xsl:template name="DynamicGraph"> 				<!-- vX - Allow generation of Dynamic Stacked Graph and DD on Detail -->
		...
		<script>
			$(function(){		
				...
				function DDGraphLegend(val){
					alert('Drill-Down Action Type 3');
					var TargetDiv = 'DD_<xsl:value-of select="$vKPI"/>_<xsl:value-of select="$vVKEYD"/>';
					<!-- File name without extra parameter -->
					<xsl:if test="$XMLFileName!='name.xml'">
						var TargetLoad = '<xsl:value-of select="$XMLFileName"/>?pRID=<xsl:value-of select="$vRID"/>&amp;pKPI=<xsl:value-of select="$vKPI"/>&amp;pEXPLORE=Yes&amp;pVKEYD=<xsl:value-of select="$vVKEYD"/>&amp;p<xsl:value-of select="$vC1"/>='+val+'&amp;pMODE=Drill-Down&amp;XXX='+Math.floor(Math.random()*1001);
					</xsl:if>
					<!-- File name with extra parameter -->
					<xsl:if test="$XMLFileParamName!='name.xml'">
						var TargetLoad = '<xsl:value-of select="$XMLFileParamName"/>&amp;pRID=<xsl:value-of select="$vRID"/>&amp;pKPI=<xsl:value-of select="$vKPI"/>&amp;pEXPLORE=Yes&amp;pVKEYD=<xsl:value-of select="$vVKEYD"/>&amp;p<xsl:value-of select="$vC1"/>='+val+'&amp;pMODE=Drill-Down&amp;XXX='+Math.floor(Math.random()*1001);
					</xsl:if>
					//alert(TargetLoad);
					loadDiv(TargetDiv,TargetLoad);
				}
				...
				$.fn.chartMRN<xsl:value-of select='$XTable_Name'/> = (function() {
					...
					$(".flot-x-axis .flot-tick-label").click(function() {
						//alert('Action 1 on X Label. Value :'+$(this).text());
						var TargetDiv = 'DD_<xsl:value-of select="$vKPI"/>_<xsl:value-of select="$vVKEYD"/>';
						<!-- File name without extra parameter -->
						<xsl:if test="$XMLFileName!='name.xml'">
							var TargetLoad = '<xsl:value-of select="$XMLFileName"/>?pRID=<xsl:value-of select="$vRID"/>&amp;pKPI=<xsl:value-of select="$vKPI"/>&amp;pEXPLORE=Yes&amp;pVKEYD=<xsl:value-of select="$vVKEYD"/>&amp;p<xsl:value-of select="$vR1"/>='+$(this).text()+'&amp;pMODE=Drill-Down&amp;XXX='+Math.floor(Math.random()*1001);
						</xsl:if>
						<!-- File name with extra parameter -->
						<xsl:if test="$XMLFileParamName!='name.xml'">
							var TargetLoad = '<xsl:value-of select="$XMLFileParamName"/>&amp;pRID=<xsl:value-of select="$vRID"/>&amp;pKPI=<xsl:value-of select="$vKPI"/>&amp;pEXPLORE=Yes&amp;pVKEYD=<xsl:value-of select="$vVKEYD"/>&amp;p<xsl:value-of select="$vR1"/>='+$(this).text()+'&amp;pMODE=Drill-Down&amp;XXX='+Math.floor(Math.random()*1001);
						</xsl:if>
						loadDiv(TargetDiv,TargetLoad);
					...
					$("#chartMRN<xsl:value-of select='$vKPI'/><xsl:value-of select='$vVKEYD'/>_<xsl:value-of select='$vVKEYD'/>").bind("plotclick", function(event, pos, item) {
						if (item) {
							//alert('Action 2 on details. Value :'+ item.series.xaxis.ticks[item.dataIndex].label + ' and '+item.series.label);
							var TargetDiv = 'DD_<xsl:value-of select="$vKPI"/>_<xsl:value-of select="$vVKEYD"/>';
							<!-- File name without extra parameter -->
							<xsl:if test="$XMLFileName!='name.xml'">
								var TargetLoad = '<xsl:value-of select="$XMLFileName"/>?pRID=<xsl:value-of select="$vRID"/>&amp;pKPI=<xsl:value-of select="$vKPI"/>&amp;pEXPLORE=Yes&amp;pVKEYD=<xsl:value-of select="$vVKEYD"/>&amp;p<xsl:value-of select="$vR1"/>='+ item.series.xaxis.ticks[item.dataIndex].label + '&amp;p<xsl:value-of select="$vC1"/>='+item.series.label+'&amp;pMODE=Drill-Down&amp;XXX='+Math.floor(Math.random()*1001);
							</xsl:if>
							<!-- File name with extra parameter -->
							<xsl:if test="$XMLFileParamName!='name.xml'">
								var TargetLoad = '<xsl:value-of select="$XMLFileParamName"/>&amp;pRID=<xsl:value-of select="$vRID"/>&amp;pKPI=<xsl:value-of select="$vKPI"/>&amp;pEXPLORE=Yes&amp;pVKEYD=<xsl:value-of select="$vVKEYD"/>&amp;p<xsl:value-of select="$vR1"/>='+ item.series.xaxis.ticks[item.dataIndex].label + '&amp;p<xsl:value-of select="$vC1"/>='+item.series.label+'&amp;pMODE=Drill-Down&amp;XXX='+Math.floor(Math.random()*1001);
							</xsl:if>
							loadDiv(TargetDiv,TargetLoad);
							<!-- window.location.href(
								clickToHref(
	...
	<xsl:template name="FromDic2Rep"> 				<!-- Template For Automatic Generation of Labels -->
		<xsl:param name="Title2S"/> 
		<xsl:param name="dictionary" select="NONE"/>
		<xsl:param name="Lang" select="'EN'"/>
		...
		<xsl:choose>
			...
			<xsl:when test="$vDic4TitleDD!='' or $vDic4TitleDD">
						...
						<xsl:when test="$tDDConfig='CIP' or $tDDConfig='CP' or $tDDConfig='IP'">
							...
							<xsl:if test="//dbquery[@id='DATA_DICTIONARY']/rows/row[@NAME=$Title2S]/@POPUP=-1">
								<script>
									...
									new Opentip("#"+unIdDD, "&lt;b>&lt;u>&lt;i>Explanation&lt;/i>&lt;/u>&lt;/b>&lt;br/><xsl:value-of select="if(//dbquery[@id='DATA_DICTIONARY']/rows/row[@NAME=$Title2S]/@HINT='' or not(//dbquery[@id='DATA_DICTIONARY']/rows/row[@NAME=$Title2S]/@HINT)) then 'No Explanation Provided' else (//dbquery[@id='DATA_DICTIONARY']/rows/row[@NAME=$Title2S]/@HINT)" />&lt;br/><xsl:if test="count(//dbquery[@id='DATA_DICTIONARY']/rows/row[@NAME=$Title2S])>1">&lt;br/>&lt;b>Warning : Multiple DataDic Entry in RepDic (Correction Required)&lt;/b>&lt;br/></xsl:if>&lt;br/>&lt;i>&lt;small>Source (DD3A) : DBWeb Node with Lang Param '<xsl:value-of select='$Lang'/>'.&lt;br/>Original Field Name : '<xsl:value-of select='$Title2S'/>'.&lt;/i>&lt;/small>", {style: "alert", target: false, tipJoint: "bottom left"});
								</script>
							</xsl:if>
						</xsl:when>
						<!-- Trapping -->
				...
				<xsl:if test="$vDicHint!=''">
					<xsl:choose>
						...
						<!-- Caption - No Icon - No Popup (Field Title) -->
						<xsl:when test="$tDDConfig='C'">
							<i><xsl:value-of select="$vDic4TitleDD"/></i>
						</xsl:when>
						<!-- Caption - (Icon) - Popup -->
						<xsl:when test="$tDDConfig='CIP' or $tDDConfig='CP' or $tDDConfig='IP'">
							<span>
								...
								<xsl:if test="$tDDConfig!='IP'">
									<span><i><xsl:value-of select="$vDic4TitleDD"/></i></span>
									<xsl:if test="count(//dbquery[@id='DATA_DICTIONARY']/rows/row[@NAME=$Title2S])>1">
										<xsl:text> </xsl:text><font color="red">(M)</font>
									</xsl:if>
								</xsl:if>
							...
							<xsl:if test="//dbquery[@id='DATA_DICTIONARY']/rows/row[@NAME=$Title2S]/@POPUP=-1">
								<script>
									var unIdDD= uniqueid();
									$('#DDToBeChanged').attr('id',unIdDD);
									new Opentip("#"+unIdDD, "&lt;b>&lt;u>&lt;i>Explanation&lt;/i>&lt;/u>&lt;/b>&lt;br/><xsl:value-of select="if(//dbquery[@id='DATA_DICTIONARY']/rows/row[@NAME=$Title2S]/@HINT='' or not(//dbquery[@id='DATA_DICTIONARY']/rows/row[@NAME=$Title2S]/@HINT)) then 'No Explanation Provided' else (//dbquery[@id='DATA_DICTIONARY']/rows/row[@NAME=$Title2S]/@HINT)" />&lt;br/><xsl:if test="count(//dbquery[@id='DATA_DICTIONARY']/rows/row[@NAME=$Title2S])>1">&lt;br/>&lt;b>Warning : Multiple DataDic Entry in RepDic (Correction Required)&lt;/b>&lt;br/></xsl:if>&lt;br/>&lt;i>&lt;small>&lt;u>Original Field Name&lt;/u> : '<xsl:value-of select='$Title2S'/>'.&lt;/i>&lt;br/>&lt;u>ILIAS Hint&lt;/u> : Nbr <xsl:value-of select="//dbquery[@id='DATA_DICTIONARY']/rows/row[@NAME=$Title2S]/@HINT_NO"/> in Screen <xsl:value-of select="//dbquery[@id='DATA_DICTIONARY']/rows/row[@NAME=$Title2S]/@FUNCTION"/>.&lt;br/>&lt;u>View Used&lt;/u> : <xsl:value-of select="//dbquery[@id='DATA_DICTIONARY']/rows/row[@NAME=$Title2S]/@VIEWNAME"/> in ILIAS (See in MITONE Screen).&lt;br/>&lt;u>Source (DD4A)&lt;/u> : DBWeb Node with Lang Param '<xsl:value-of select='$Lang'/>'.&lt;/small>", {style: "alert", target: false, tipJoint: "bottom left"});
								</script>
							</xsl:if>
						</xsl:when>
						<!-- Trapping -->
			...
			<xsl:when test="$vDic4TitleLDD!='' or $vDic4TitleLDD">
				<xsl:choose>
					...
					<!-- Caption - No Icon - No Popup (Field Title) -->
					<xsl:when test="$tDDConfig='C'">
						<i><xsl:value-of select="$vDic4TitleLDD"/></i>
					</xsl:when>
					<!-- Caption - (Icon) - Popup -->
					<xsl:when test="$tDDConfig='CIP' or $tDDConfig='CP' or $tDDConfig='IP'">
						<span id="DDToBeChanged">
							<xsl:if test="$tDDConfig!='IP'">
								<span><i><xsl:value-of select="$vDic4TitleLDD"/></i></span>
						...
						<script>
							var unIdDD= uniqueid();
							$('#DDToBeChanged').attr('id',unIdDD);
							new Opentip("#"+unIdDD, "&lt;b>&lt;u>&lt;i>Explanation&lt;/i>&lt;/u>&lt;/b>&lt;br/><xsl:value-of select="if($dictionary//Label[@ID=$Title2S]/PopupText[upper-case(@lang)=$Lang]='' or not($dictionary//Label[@ID=$Title2S]/PopupText[upper-case(@lang)=$Lang])) then 'No Explanation Provided' else ($dictionary//Label[@ID=$Title2S]/PopupText[upper-case(@lang)=$Lang])" />&lt;br/><xsl:if test="count($dictionary//Label[@ID=$Title2S]/LabelText[upper-case(@lang)=$Lang])>1">&lt;br/>&lt;b>Warning : Multiple DataDic Entry in RepDic (Correction Required)&lt;/b>&lt;br/></xsl:if>&lt;br/>&lt;i>&lt;small>Source (DD2A) : Local XML (...RepDic.xml) with Lang Param '<xsl:value-of select='$Lang'/>'.&lt;br/>Original Field Name : '<xsl:value-of select='$Title2S'/>'.&lt;/i>&lt;/small>", {style: "alert", target: false, tipJoint: "bottom left"});
						</script>
					</xsl:when>
					<!-- Trapping -->
					<xsl:otherwise>
				...
				<xsl:choose>
					...
					<xsl:when test="$tDDConfig='CIP' or $tDDConfig='CP' or $tDDConfig='IP'">
						...
						<script>
							var unIdDD= uniqueid();
							$('#DDToBeChanged').attr('id',unIdDD);
							new Opentip("#"+unIdDD, "&lt;b>&lt;u>&lt;i>Field Not Defined in Data Dictionary (DBWeb) and in RepDic (.xml)&lt;/i>&lt;/u>&lt;/b>&lt;br/>No Label and Pop-Up defined in Report Dictionary. Help Us to define it.&lt;br/>&lt;br/>&lt;i>&lt;small>Source (DD1A) : No DBWeb DD And No RepDic with Lang Param '<xsl:value-of select='$Lang'/>'.&lt;br/>Original Field Name : '<xsl:value-of select='$Title2S'/>'.&lt;/i>&lt;/small>", {style: "alert", target: false, tipJoint: "bottom left"});
						</script>
					</xsl:when>
					<!-- Trapping -->
					<xsl:otherwise>
	...
	<xsl:template name="TNotif">                  	<!-- v2 - Used To SHow Debug Messages-->
				    ...
				    <!-- Notification Settings -->
				      ...
				      <script type="text/javascript">
				         ...
				         // Notifications
							...
							setTimeout(function()
				               <xsl:choose>
				                  <xsl:when test="$tType=2">{toastr.success("<xsl:value-of select="$tMsg"/>", <xsl:choose><xsl:when test="$tTitle='None'">null</xsl:when><xsl:otherwise>"<xsl:value-of select="$tTitle"/>"</xsl:otherwise></xsl:choose>, opts);}, <xsl:value-of select="$tAppear"/>);</xsl:when>
				                  <xsl:when test="$tType=3">{toastr.warning("<xsl:value-of select="$tMsg"/>", <xsl:choose><xsl:when test="$tTitle='None'">null</xsl:when><xsl:otherwise>"<xsl:value-of select="$tTitle"/>"</xsl:otherwise></xsl:choose>, opts);}, <xsl:value-of select="$tAppear"/>);</xsl:when>
				                  <xsl:when test="$tType=4">{toastr.error("<xsl:value-of select="$tMsg"/>", <xsl:choose><xsl:when test="$tTitle='None'">null</xsl:when><xsl:otherwise>"<xsl:value-of select="$tTitle"/>"</xsl:otherwise></xsl:choose>, opts);}, <xsl:value-of select="$tAppear"/>);</xsl:when>
				                  <xsl:otherwise>{toastr.info("<xsl:value-of select="$tMsg"/>", <xsl:choose><xsl:when test="$tTitle='No Title'">null</xsl:when><xsl:otherwise>"<xsl:value-of select="$tTitle"/>"</xsl:otherwise></xsl:choose>, opts);}, <xsl:value-of select="$tAppear"/>);</xsl:otherwise>
				               </xsl:choose>
				      </script>	
	...
	<xsl:template name="TMpopup">                  	<!-- v2 - Used To SHow Debug Messages-->
	...
	<xsl:template name="Decode_FDC_MultipleValue"> 	<!-- v2 - Allow Decoding of MultipleValue in 1 Field (SPE and FDC) -->
	    <xsl:param name="tText" select="'aa,bb,cc'"/>
	    <xsl:param name="tDelim" select="','" />
		<xsl:param name="tStart" select="1"/> 		<!-- Use 1 -->
	    <xsl:param name="tPos" select="4"/>
	    <xsl:param name="tLength" select="5"/>
		<xsl:choose>
...
<!-- User For Learning Packet (Sample)-->
	<xsl:template name="RepObjective_InfoBox"> 		<!-- Generation of Report Objective InfoBox based on Local RepDic -->
		...
		<xsl:choose>
			<xsl:when test="$RepObj='' or not($RepObj)">
				Do Nothing because no Objactive Defined
			</xsl:when>
			<xsl:otherwise>
				<div id="Explain">
	...
	<xsl:template name="Body_Start"> 					<!-- v2 - Template to be placed just after Body Tag-->	
	...
	<xsl:template name="Header">     					<!-- v2 - Global Report Header Version 2 (without EXPLORER Page) -->
	...
	<xsl:template name="Report_Infos">     				<!-- v2 - Report Info Header (Top-Left) Version 2 (without EXPLORER Page) -->
		<xsl:param name="tLocal" select="'NO'"/>
		<xsl:param name="tRepDic" select="'NO'"/>
		<xsl:param name="tLanguage" select="'EN'"/>
		<!-- Variable declaration -->
	...
	<xsl:template name="Accuracy_Indic">				<!-- v2 - Report Quality-Accuracy Markers in  -->
	...
	<xsl:template name="Report_Prompts">     			<!-- v2 - Report Prompts (Top-Right) Version 2 (without EXPLORER Page) -->
		<xsl:param name="tLocal" select="'NO'"/>
		<xsl:param name="tRepDic" select="'NO'"/>
		<xsl:param name="tLanguage" select="'EN'"/>
		<!-- Variable declaration -->
	...
	<xsl:template name="List_Of_Prompts">				<!-- v2 - List Of Prompts (called in All Reports) -->
	...
	<xsl:template name="Body_End"> 						<!-- V2 - Template to be placed just before end Body Tag -->
		<xsl:param name="tDebug" select="'NO'"/> 		<!-- YES or No (Default) - Depreacated Do Not Use -->
		<xsl:param name="tRepDic" select="'NO'"/> 		<!-- Dictionary ? Type relative Path or No (Default) -->
		<xsl:param name="tLocal" select="'NO'"/> 		<!-- Yes for Local and NO (Default) for MR Node - Depreacated Do Not Use -->
		<xsl:param name="tLanguage" select="$vLANG"/> 	<!-- EN (Default) - English/FR - Français/NL - Nederlands -->
		
		<!-- Used ? - Check DataDic Incomplete - DataDic
		<script type="text/javascript" src="/Default2/MRN_Prod/extras/alasql/alasql.min.js"></script>
		<script type="text/javascript" src="/Default2/MRN_Prod/extras/alasql/xlsx.core.min.js"></script>
		
		<table id="FillDD" style="display:none">
	        <thead>
	        	<tr><th>Type</th><th>ILIAS Hint</th><th>Type</th><th>Field Name</th><th>Lang</th><th>Caption</th><th>Hint</th></tr>
	        </thead>
	        <tbody/>
	    </table>
		<div id="ExplainDDNotif" style="display:none">
			Report data Dictionary is incomple.<br/>Click <span style="cursor:pointer" onclick='GenerateDataDic();'><b><u><font color="red" >here</font></u></b></span> to generate a XLS File which contains all Fields to define in this Report Data dictionary.<br/> Your Help will be appreciated.
		</div>
		<script>
			var DDC=0;
	...
	<xsl:template name="Footer"> 						<!-- v2 - Logo Footer -->		
		<xsl:param name="tDebug" select="'NO'"/> 		<!-- YES or No (Default) -->
		<xsl:param name="tRepDic" select="'NO'"/> 		<!-- Dictionary ? Type relative Path or No (Default) -->
		...
		<xsl:for-each select = "//dbquery[@id='Report_Info']/rows/row[1]">
			<!-- Replaced by Modal Load 
			<xsl:if test="(//dbquery[1]/descriptor/parameters/param[@name='pKPI']/@value and //dbquery[1]/descriptor/parameters/param[@name='pEXPLORE']/@value='YES' and //dbquery[1]/descriptor/parameters/param[@name='pHIST']/@value='YES')">
				<br/>
					<center>
						<div Id="KPI_HISTORICAL">KPI Historical To Load</div>
					</center>
					<script>
						var TargetDiv = 'KPI_HISTORICAL';
						var TargetLoad = 'IL_MR_KPI_HISTORICAL.xml?pLANG=<xsl:value-of select="//dbquery[1]/descriptor/parameters/param[@name='pLANG']/@value"/>&amp;pKPI=<xsl:value-of select="//dbquery[1]/descriptor/parameters/param[@name='pKPI']/@value"/>&amp;pVKEYD=<xsl:value-of select="//dbquery[1]/descriptor/parameters/param[@name='pVKEYD']/@value"/>&amp;XXX='+Math.floor(Math.random()*1001);
						// Chargement
						loadDiv(TargetDiv,TargetLoad);
					</script>
				<br/>
	...
	<xsl:template name="Report2Improve"> 				<!-- v2 - Bloc Improve Report (in Report Bottom) - Depreacated Do Not USE -->
		<xsl:param name="tRepDic" select="'NO'"/> 		<!-- Dictionary ? Type relative Path or No (Default) -->
		<xsl:param name="tLocal" select="'NO'"/> 		<!-- Yes for Local and NO (Default) for MR Node -->
		<xsl:param name="tLanguage" select="'EN'"/>
	...
	<xsl:template name="DBWeb_Decryptor"> 				<!-- v2 - Report Error Decriptor Applic -->
		...
		<div id="CPN_Report_DBWebDescriptor" class="modal fade" role="dialog">
	        <div class="modal-dialog modal-lg">
	            ...
	            <div class="modal-content">
	                ...
	                <div class="modal-body">
	                    <xsl:variable name="NBR_DBWEB_DECRYPT"><xsl:value-of select="count(//dbquery[@id!='Report_Info' and @id!='Report_Prompt' and @id!='TEMPLATE' and @id!='KPI']/descriptor/querystring)"/></xsl:variable>
	                    ...
	                    <div class="row">
	                        <div class="col-md-12">
	                            <p style="font-style:italic">
	                            	This page summarize all <b>Queries Used (<xsl:value-of select="$NBR_DBWEB_DECRYPT"/>)</b> in this Report. <br/>
		                            <ul>
		                            	<li>Node Setting : '<xsl:value-of select="$vShowDBWeb"/>' </li>
		                            	<li>Report Setting : '<xsl:value-of select="//dbquery[@id='Report_Info']/rows/row/@dbn_rep_dbweb"/>'</li>
		                            	<li>Errors in Report : <xsl:value-of select="if(count(//dbquery/error)=0) then 'No Errors' else count(//dbquery/error)"/></li>
		                            </ul>
		                            	 Result ... So Queries Descriptor Authorization is '<xsl:value-of select="$SHOW_DBWEB_DECRYPT"/>' 
		                            	 <xsl:if test="count(//dbquery/error)!=0"> But you will see Queries in errors</xsl:if>
	                            </p>
	                        </div>
	                    </div>
	                    <!-- Calculate Table if Yes or Error -->
	                    <xsl:if test="$SHOW_DBWEB_DECRYPT='YES' or count(//dbquery/error)!=0">
		                    <div class="row">
								<!--div style="display:none"-->
									<xsl:attribute name="id">D_<xsl:value-of select="$tUQD"/></xsl:attribute>
									<!-- Set parameter Show -->
									<xsl:choose>
									...
									<table border="0" align="center">
										<thead>
											<th align="center"  width="5%" style='padding:3px 1px 0px 1px'>
												<xsl:element name="a">
													<xsl:attribute name="onclick">if(<xsl:value-of select="$tUQD"/>.style.display=='none'){<xsl:value-of select="$tUQD"/>.style.display='block';img<xsl:value-of select="$tUQD"/>.src="/<xsl:value-of select='$vHtDocsConfig'/>/CPN/img/down-bl.png"} else {<xsl:value-of select="$tUQD"/>.style.display='none';img<xsl:value-of select="$tUQD"/>.src="/<xsl:value-of select='$vHtDocsConfig'/>/CPN/img/up-bl.png"};
													</xsl:attribute>
													<xsl:attribute name="style"> style="cursor:pointer;</xsl:attribute>
													<xsl:element name="img">
														<xsl:attribute name="id">img<xsl:value-of select="$tUQD"/></xsl:attribute>   
														<xsl:attribute name="style">cursor:pointer;margin-left:5px;margin-right:5px</xsl:attribute>  
														<xsl:attribute name="src">/<xsl:value-of select='$vHtDocsConfig'/>/CPN/img/up-bl.png</xsl:attribute>   
													</xsl:element>								
												</xsl:element>
											</th>
											<th>
												<xsl:choose>
													 <!-- No Errors -->
													 <xsl:when test="count(//dbquery/error) = 0">
														<xsl:value-of select="$NBR_DBWEB_DECRYPT"/> Queries used without error in this Report
													 </xsl:when>
													 <!-- Errors -->
													 <xsl:when test="count(//dbquery/error) != 0">
												...
												<xsl:if test="$vMODE!=''"> (in Mode : <xsl:value-of select="$vMODE"/>)</xsl:if>
											</th>
										</thead>
									</table>
									<center>
										<div>
											<xsl:attribute name="id"><xsl:value-of select="$tUQD"/></xsl:attribute>
											<xsl:attribute name="style">display:none</xsl:attribute>
											<td style="background-color:#ffffff;">
													...
													<tbody>
														<xsl:choose>
															<xsl:when test="$SHOW_DBWEB_DECRYPT = 'YES' or //dbquery[@id='Report_Info']/descriptor/parameters/param[@name='pKPI']/@value!=''">
																<xsl:for-each select = "//dbquery[@id!='Report_Info' and @id!='Report_Prompt' and @id!='TEMPLATE' and @id!='KPI']">		
																	<tr>
																		<xsl:choose>
																			<xsl:when test="count(rows/row)=0 and count(error)!=1">
																				...
																				<td bgcolor="#9C9E4B">
																					<b>
																						...
																						<xsl:if test="@id='DATA_DICTIONARY'">
																							<span onclick="alert('Later ...');" style="color:red;Cursor:pointer"> (View)</span>
																						</xsl:if>
																					</b>
																				</td>
																				...
																				<td align="center" >
																					<b>
																						...
																						<span style="Cursor:pointer">
																							<xsl:attribute name="onclick">$('#<xsl:value-of select="$tUQD"/>_Fields_<xsl:value-of select="@id"/>').show()</xsl:attribute>
																							<font color="blue"> (Show)</font>
																						</span>
																					</b>
																				</td><td align="center" ><b><xsl:value-of select="count(rows/row)"/></b></td>
																			</xsl:when>
																			<xsl:when test="count(rows/row)>=0 and count(error)!=1">
																	...
																	<tr style="display:none">
																		<xsl:attribute name="id"><xsl:value-of select="$tUQD"/>_Fields_<xsl:value-of select="@id"/></xsl:attribute>
																		<td>
																			Fields :
																		</td>
																		<td colspan="4">
															...
															<xsl:otherwise>
																<xsl:for-each select = "//dbquery/error/cause">		
																	...
																	<tr>
																		<td align="left" colspan="4">
																			<b><xsl:value-of select="substring(../message,1,400)"/>...</b>
																		</td>
																	</tr>
																</xsl:for-each>
															</xsl:otherwise>
	...
	<xsl:template name="DataDic_Decryptor"> 			<!-- v2 - Data Dictionary decriptor -->
...
<!-- For KPI -->
	<xsl:template name="KPI_Toolbar"> 							<!-- v2 - Template For KPI Version 2 (without EXPLORER Page) -->
		...
		<xsl:param name="tLanguage" select="'EN'"/>
		<!-- KPI Toolbar - Old Version -->
		<!--
		<xsl:element name="div">
			<xsl:attribute name="id">D_KPI<xsl:value-of select='$vKPI'/>_<xsl:value-of select='$tVKEYD'/></xsl:attribute>
			<xsl:attribute name="style">display:block</xsl:attribute>
			<table border="0" align="center" width="95%" class="display">
				<thead bgcolor="#AB9D7B">
					<th align="center"  style='padding:3px 1px 0px 1px' width="5%">
						<xsl:element name="a">
							<xsl:attribute name="onclick">
								if(KPIDETREP<xsl:value-of select='$tKPI'/>_<xsl:value-of select='$tVKEYD'/>.style.display=='none'){KPIDETREP<xsl:value-of select='$tKPI'/>_<xsl:value-of select='$tVKEYD'/>.style.display='block';imgKPI<xsl:value-of select='$tKPI'/>_<xsl:value-of select='$tVKEYD'/>.src='/<xsl:value-of select='$vHtDocsConfig'/>/CPN/img/down-bl.png'} else {KPIDETREP<xsl:value-of select='$tKPI'/>_<xsl:value-of select='$tVKEYD'/>.style.display='none';imgKPI<xsl:value-of select='$tKPI'/>_<xsl:value-of select='$tVKEYD'/>.src='/<xsl:value-of select='$vHtDocsConfig'/>/CPN/img/up-bl.png'};
							</xsl:attribute>
							<xsl:attribute name="style"> style=cursor:pointer;</xsl:attribute>
							<xsl:element name="img">
								<xsl:attribute name="id">imgKPI<xsl:value-of select='$tKPI'/>_<xsl:value-of select='$tVKEYD'/></xsl:attribute>
								<xsl:attribute name="style">cursor:pointer;margin-left:5px;margin-right:5px</xsl:attribute>
								<xsl:attribute name="src">/<xsl:value-of select='$vHtDocsConfig'/>/CPN/img/down-bl.png</xsl:attribute>
							</xsl:element>
						</xsl:element>
					</th>
					<th>
					...
					<th width="8%" align="center">
						<a href="http://icp.idcn.mil.intra/bedeffr/index.php/Report_KPI{$tKPI}" target="KPI_DOC_WIKI" style="text-decoration: none;color: #0592FA;">Fr</a>			
						<xsl:text> </xsl:text><img src="/{$vHtDocsConfig}/CPN/img/Flat/file-folder-icon.png" width="18px"/><xsl:text> </xsl:text>
						<a href="http://icp.idcn.mil.intra/bedefnl/index.php/Report_KPI{$tKPI}" target="KPI_DOC_WIKI" style="text-decoration: none;color: #0592FA;">Nl</a>
					</th>
					...
					<xsl:if test="$vEXPLORE='NO'">
						<TH width="8%">
							<a href="IL_MR_KPI{$vKPI}.xml?pRID={$vRID}&amp;pVKEYD={$vVKEYD}&amp;pKPI={$vKPI}&amp;pLANG={$tLanguage}&amp;pHIST=YES&amp;pEXPLORE=YES" target="_blank" style="text-decoration: none;color: #0592FA;">Explore</a>	
						</TH>	
					</xsl:if>	
				</thead>
				<tbody/>
					...
					<ul class="nav navbar-nav navbar-right">
						...
						<li class="dropdown">
				          ...
				          <ul class="dropdown-menu">
				            ...
				            <li>
				            	<a href="/LRF/XMLWeb/ProcessDescriptor/descriptor/KPI/IL_MR_KPI/IL_MR_KPI_HISTORICAL.xml?pRID=&amp;pVKEYD={$tVKEYD}&amp;pKPI={$tKPI}&amp;pLANG={$vLANG}" data-toggle="modal" data-target="#Modal_{$tKPI}_{$tVKEYD}" role="button">
									Historical
								</a> 
							</li>
							<li>
				        ...
				        <!-- Historical -->
			        	<p class="navbar-text">
			        		<b><a href="/LRF/XMLWeb/ProcessDescriptor/descriptor/KPI/IL_MR_KPI/IL_MR_KPI_HISTORICAL.xml?pRID=&amp;pVKEYD={$tVKEYD}&amp;pKPI={$tKPI}&amp;pLANG={$vLANG}" data-toggle="modal" data-target="#Modal_{$tKPI}_{$tVKEYD}" role="button"><span class="glyphicon glyphicon-stats" aria-hidden="true"></span> Historical</a></b>
						</p>
				        <!-- Explore KPI Hyperlink -->
				        <xsl:if test="$vEXPLORE='NO'">
				        	<p class="navbar-text">
								<b><a href="IL_MR_KPI{$vKPI}.xml?pRID={$vRID}&amp;pVKEYD={$vVKEYD}&amp;pKPI={$vKPI}&amp;pLANG={$tLanguage}&amp;pHIST=YES&amp;pEXPLORE=YES" target="_blank" role="button"><span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span> Explore</a></b>
							</p>
						</xsl:if>
				      </ul>
				</div><!-- /.navbar-collapse -->
