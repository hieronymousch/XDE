...
<!--**********************************************
	...
	********************************************** -->
	...
	<xsl:template name="ReportSecurity"> 			<!-- Security Mechanism -->
		...
		<!-- If Security is activated ... Check if URL is matching with RepId Notification only for Main Report -->
			...
			<xsl:if test="$tSecType!='None'">
				...
				<script>
					CheckURL('<xsl:value-of select="$vURL"/>','<xsl:value-of select="$vPROFILE"/>');
				</script>
			</xsl:if>
			...
			<xsl:choose>
			    ...
			    <xsl:when test="$tSecType='ReportDM'">
			    	<xsl:choose>
						...
						<xsl:otherwise>
							<xsl:call-template name="TNotif">
								<xsl:with-param name="tMsg" select="'DataModel based Contributor Report Security has been activated on this Report and you are granted to access this Report.'"/>
			...
			<xsl:when test="$dT_Type='99'"> 	<!-- For Std Table Result, all features activated (Version to Copy) -->
				<script>
					...
					var oTable_dT_<xsl:value-of select="$Table_Name"/> = $('#dT_<xsl:value-of select="$Table_Name"/>').on('error.dt', function(e, settings, techNote, message){
							MRNNotif('4','DataTables error',message,30,'ALL');
								...
								buttons: [
									{
										text: 'Reset',
										action: function(){
											heatMap(255,255,255,'dT_<xsl:value-of select="$Table_Name"/>','reset');
										}
									},
									{
										text: 'Global',
										action: function() {
											heatMap(118,246,68,'dT_<xsl:value-of select="$Table_Name"/>','global');
										}
									},
									{
										text: 'By Rows',
										action: function() {
											heatMap(118,246,68,'dT_<xsl:value-of select="$Table_Name"/>','rows');
										}
									},
									{
										text: 'By Columns',
										action: function() {
											heatMap(118,246,68,'dT_<xsl:value-of select="$Table_Name"/>','columns');
										}
									}
								]
					        }
	...
	<xsl:template name="DynamicXTable"> 			<!-- Dynamic Crosstable and DD on Detail  -->
		...
		<xsl:param name="Lang" select="'EN'"/>
		<xsl:param name="RepDic" select="'N'"/>				<!-- Use of Report Dictionnary. Default 'N' / 'Y' -->
		<xsl:param name="dT_HeatMap" select="$vNodeHeatMap"/>			<!-- Show heatmap button. Default 'N' / 'Y' -->
	...
	<xsl:template name="PivotTable">
		<xsl:param name="tDBWeb" select="'PIVOT'"/>
    	...
    	<div id="output" style="width: 90%;"></div>
	</xsl:template>
	...
	<xsl:template name="Footer"> 					<!-- Report Footer -->		
