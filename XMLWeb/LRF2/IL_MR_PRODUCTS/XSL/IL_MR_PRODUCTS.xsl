    ...
    <meta name="author" content="JF"/>
		...
		<body>
			...
			<xsl:call-template name="Generic_Table_DBWEB">
				...
				<xsl:with-param name="UdTN" select="'IL_MR_PRODUCT'"/>
...
<xsl:template match="@LotF" mode="genericTableCel">                  
    <td align="center">
      <xsl:choose>
        ...
        <xsl:otherwise><span class="label label-warning"><xsl:value-of select="."/></span></xsl:otherwise>
      </xsl:choose>
    </td>
</xsl:template>
