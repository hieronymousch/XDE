...
<xsl:variable name="vMONTH" select="//dbquery[1]/descriptor/parameters/param[@name='pMONTH']/@value"/>
		...
		<body>
					    ...
					    <h3>
					    	...
					    	<div style="width:100%; height:500px;">
								...
								<xsl:variable name="vCodeGR1">
									<xsl:choose>
										<xsl:when test="$vGR1='MONTH'">AS</xsl:when>
										<xsl:otherwise>NONE</xsl:otherwise>
									</xsl:choose>
								</xsl:variable>
								<xsl:variable name="vCodeGR2">
									<xsl:choose>
										<xsl:when test="$vGR2='MONTH'">AS</xsl:when>
										<xsl:otherwise>NONE</xsl:otherwise>
									</xsl:choose>
								</xsl:variable>
								<xsl:variable name="vCodeGC1">
									<xsl:choose>
										<xsl:when test="$vGC1='MONTH'">AS</xsl:when>
										<xsl:otherwise>NONE</xsl:otherwise>
									</xsl:choose>
								</xsl:variable>
								<!-- Call Dynamic Graph -->
			...
			<div>
				...
				<!--<xsl:if test="$vMODE='Drill-Down'">-->
					<xsl:variable name="TNameDD">DOSSIER_DETAILS</xsl:variable>
					<xsl:call-template name="Generic_Table_DBWEB">
...
<xsl:template match="row">
	...
	<xsl:choose>
		<xsl:when test="$Detail_Data='SPE_01_XTAB_ASS'">
			...
			<xsl:variable name="vXMLFileName">IL_MR_DOSS_DISTR.xml</xsl:variable>
			<!-- Determine Which LLOOK Code is associated with values for XTAB -->
			<xsl:variable name="vCodeR1">
				<xsl:choose>
					<xsl:when test="$vR1='MONTH'">AS</xsl:when>
					<xsl:otherwise>NONE</xsl:otherwise>
				</xsl:choose>
			</xsl:variable>
			<xsl:variable name="vCodeR2">
				<xsl:choose>
					<xsl:when test="$vR2='MONTH'">AS</xsl:when>
					<xsl:otherwise>NONE</xsl:otherwise>
				</xsl:choose>
			</xsl:variable>
			<xsl:variable name="vCodeC1">
				<xsl:choose>
					<xsl:when test="$vC1='MONTH'">AS</xsl:when>
					<xsl:otherwise>NONE</xsl:otherwise>
				</xsl:choose>
			</xsl:variable>
