...
<!-- Generate HTML5 WebPage -->
	...
	<html>
		<head>
			...
			<title>Electronic Ordering Batch</title>
		</head>
		<body>
			...
			<xsl:if test="count(//dbquery[@id='IL_MR_EOB']/rows/row)=0">
				<center>
					<h2>This Report Requires the PCOIN Extractions in your Local DB Node.</h2>
				</center>
			</xsl:if>
			...
			<center>
			<h2>
			<a href="mailto:{//dbquery[@id='IL_MR_EOB_CONTACT']/rows/row/@*[local-name() = 'MailContractor']}&amp;cc={//dbquery[@id='IL_MR_EOB_CONTACT']/rows/row/@*[local-name() = 'MailPurOrg']};{//dbquery[@id='IL_MR_EOB_CONTACT']/rows/row/@*[local-name() = 'MailMatMan']};{//dbquery[@id='IL_MR_EOB_CONTACT']/rows/row/@*[local-name() = 'MailLeadServ']}?subject=Contract {//dbquery[@id='IL_MR_EOB_CONTACT']/rows/row/@*[local-name() = 'contract']} // Order {//dbquery[@id='IL_MR_EOB_CONTACT']/rows/row/@*[local-name() = 'ordernr']}&amp;body=See%20order%20in%20attachment">
			  Create Email </a>
			  </h2>
			</center>
			<xsl:call-template name="Generic_Table_DBWEB">
				<xsl:with-param name="DBWEB_Name" select="'IL_MR_EOB_CONTACT'"/>
				<xsl:with-param name="UdTN" select="'IL_MR_EOB_CONTACT'"/>
				<xsl:with-param name="Node_Conf" select="$RepConf"/>
				<xsl:with-param name="ForcedTitle" select="'Contacts for this EOB'"/>	
			</xsl:call-template>
			
			
			
			<xsl:call-template name="Generic_Table_DBWEB">
				<xsl:with-param name="DBWEB_Name" select="'IL_MR_EOB'"/>
				<xsl:with-param name="UdTN" select="'IL_MR_EOB_DETAILS'"/>
				<xsl:with-param name="Node_Conf" select="$RepConf"/>
				<xsl:with-param name="dT_Type" select="'99'"/>
				<xsl:with-param name="Lang" select="$vLANG"/>
				<xsl:with-param name="RepDic" select="'Y'"/> 
				<xsl:with-param name="dictionary" select="$vRepDic"/> 	
				<xsl:with-param name="ForcedTitle" select="'Order file:'"/>	
