...
<xsl:variable name="dTColHidden">'9'</xsl:variable>
...
<!-- Generate HTML5 WebPage -->
	...
	<html>
		...
		<body>
			...
			<xsl:if test="count(//dbquery[@id='IL_MR_PREQ']/rows/row)=0">
				<center>
			...
			<center> 
				<h1>PREQ's (all statusses)</h1>
			</center>	
			<br/>
			
			<xsl:call-template name="Generic_Table_DBWEB">
				<xsl:with-param name="DBWEB_Name" select="'IL_MR_PREQ_DETAILS_PUR_REQ'"/>
				<xsl:with-param name="UdTN" select="'IL_MR_PREQ_DETAILS_PUR_REQ'"/>
				<xsl:with-param name="Node_Conf" select="$RepConf"/>
				<xsl:with-param name="dT_Type" select="'99'"/>
				<xsl:with-param name="Lang" select="$vLANG"/>
				<xsl:with-param name="RepDic" select="'Y'"/> 
				<xsl:with-param name="dictionary" select="$vRepDic"/> 	
				<xsl:with-param name="ForcedTitle" select="'PREQ Addition information'"/>	
				...
				<!--  IL_MR_KPI5029.xml?pRID=1029&pKPI=5029&pVKEYD=VS&pYEAR=2015&pSOURCE=INVPLAN&pPROG=KIM&pPREPTYPE=ALL&pPREQ=L6A7&pLANG=EN&pMODE=Drill-Down2&XXX=425 -->
			...
			<xsl:call-template name="Generic_Table_DBWEB">
				<xsl:with-param name="DBWEB_Name" select="'IL_MR_PREQ_DETAILS_PUR_REQ_VER'"/>
				<xsl:with-param name="UdTN" select="'IL_MR_PREQ_DETAILS_PUR_REQ_VER'"/>
				<xsl:with-param name="Node_Conf" select="$RepConf"/>
				<xsl:with-param name="dT_Type" select="'99'"/>
				<xsl:with-param name="Lang" select="$vLANG"/>
				<xsl:with-param name="RepDic" select="'Y'"/> 
				<xsl:with-param name="dictionary" select="$vRepDic"/> 	
				<xsl:with-param name="ForcedTitle" select="'PREQ Versions'"/>	
				<xsl:with-param name="ForceFooter" select="'Y'"/> 
				<!-- Hide Columns Nbr in dT. Default is 'None' / Example '2,3'. Done via variable declared at beginning of xsl -->
				<xsl:with-param name="Col_Hidden" select="$dTColHidden"/>	
				<!-- Table Sorting on Column Nbr in dT. Default 'None' / Example '1-asc' or '2-desc'. Done via variable declared at beginning of xsl -->				
			...
			<xsl:call-template name="Generic_Table_DBWEB">
				<xsl:with-param name="DBWEB_Name" select="'IL_MR_DOSS'"/>
				<xsl:with-param name="UdTN" select="'IL_MR_DOSS'"/>
				<xsl:with-param name="Node_Conf" select="$RepConf"/>
				<xsl:with-param name="dT_Type" select="'99'"/>
				<xsl:with-param name="Lang" select="$vLANG"/>
				<xsl:with-param name="RepDic" select="'N'"/> 
				<xsl:with-param name="dictionary" select="$vRepDic"/> 	
				<xsl:with-param name="ForcedTitle" select="'Dossier'"/>			
			</xsl:call-template>
						
			<xsl:call-template name="Generic_Table_DBWEB">
				<xsl:with-param name="DBWEB_Name" select="'IMPUTATION'"/>
				<xsl:with-param name="UdTN" select="'IMPUTATION'"/>
				<xsl:with-param name="Node_Conf" select="$RepConf"/>
				<xsl:with-param name="dT_Type" select="'99'"/>
				<xsl:with-param name="Lang" select="$vLANG"/>
				<xsl:with-param name="RepDic" select="'N'"/> 
				<xsl:with-param name="dictionary" select="$vRepDic"/> 	
				<xsl:with-param name="ForcedTitle" select="'Imputation'"/>			
			</xsl:call-template>
