...
<xsl:variable name="vRepDic" select="document('../INV_TRANSFERS_RepDic.xml')" />
<!--
...
<xsl:template match='/'>	
	...
	<meta name="author" content="Jeroen Franssen"/>
		...
		<body>
			...
			<xsl:if test="count(//dbquery[@id='INV_TRANSFERS']/rows/row)=0">
				<center>
					<h2>This Report Requires the MRQR_TRANSACTIONS_OPEN Extraction in your Local DB Node.</h2>
				</center>
			</xsl:if>
			...
			<xsl:call-template name="Generic_Table_DBWEB">
				<xsl:with-param name="DBWEB_Name" select="'INV_TRANSFERS'"/>
				<xsl:with-param name="Lang" select="$vLANG"/>
				<xsl:with-param name="RepDic" select="'Y'"/> 
				<xsl:with-param name="dictionary" select="$vRepDic"/> 	
				<xsl:with-param name="ForcedTitle" select="'Open Transfers'"/>	
