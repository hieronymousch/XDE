...
<xsl:variable name="vRepDic" select="document('../INV_REQUISITIONS_RepDic.xml')" />
<!--
...
<xsl:template match='/'>	
	...
	<meta name="author" content="Jeroen Franssen"/>
		<head>
		...
		<body>
			...
			<xsl:if test="count(//dbquery[@id='INV_REQUISITIONS']/rows/row)=0">
				<center>
					<h2>This Report Requires the MRQR_REQUISITIONS Extraction in your Local DB Node.</h2>
				</center>
			</xsl:if>
			...
			<xsl:call-template name="Generic_Table_DBWEB">
				<xsl:with-param name="DBWEB_Name" select="'INV_REQUISITIONS'"/>
				<xsl:with-param name="UdTN" select="'INV_REQUISITIONS'"/>

				<xsl:with-param name="RepDic" select="'Y'"/> 
				<xsl:with-param name="dictionary" select="$vRepDic"/> 	
				<xsl:with-param name="ForcedTitle" select="'Requisitions'"/>	

			<!-- <xsl:with-param name="DDURL" select="'../../INV_COLLI/INV_REQUISITIONS_DETAILS.xml?Mode=Drill-Down&amp;pREQNO='"/> -->
			<xsl:with-param name="DDURL" select="'http://mrnode.mil.intra/LRF/XMLWeb/ProcessDescriptor/descriptor/LRF/IL_MR_SSCC/IL_MR_SSCC_DETAIL.xml?pRID=15&amp;?Mode=Drill-Down&amp;pREQNO='"/>
