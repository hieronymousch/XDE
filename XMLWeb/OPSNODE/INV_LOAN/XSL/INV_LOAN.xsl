...
<xsl:variable name="vRepDic" select="document('../INV_LOAN_RepDic.xml')" />
<!--
...
<xsl:template match='/'>	
	...
	<meta name="author" content="Jeroen Franssen"/>
		...
		<body>
			...
			<xsl:if test="count(//dbquery[@id='INV_LOAN']/rows/row)=0">
				<center>
					<h2>This Report Requires the MRQR Loan Extraction in your Local DB Node.</h2>
				</center>
			</xsl:if>
			...
			<xsl:call-template name="Generic_Table_DBWEB">
				<xsl:with-param name="DBWEB_Name" select="'INV_LOAN'"/>
				<xsl:with-param name="UdTN" select="'INV_LOAN'"/>
				<xsl:with-param name="RepDic" select="'Y'"/> 
				<xsl:with-param name="dictionary" select="$vRepDic"/> 	
			    <xsl:with-param name="ForcedTitle" select="'Loanfiles'"/>
				<xsl:with-param name="DDURL" select="'INV_LOAN_DD.xml?pDETAILS=on&amp;Mode=Drill-Down&amp;pLOAN='"/> 
