...
<!-- Generate HTML5 WebPage -->
	...
	<html>
		...
		<body>
			...
			<xsl:if test="count(//dbquery[@id='EFFECTIVENESS_GLOBAL']/rows/row)=0">
				<center>
					<h2>This Report Requires the MRQR mrqr_mission_units_material Extraction in your Local DB Node.</h2>
				</center>
			</xsl:if>
			...
			<xsl:call-template name="Generic_Table_DBWEB">
				...
				<xsl:with-param name="UdTN" select="'effectiveness'"/>
				<xsl:with-param name="Node_Conf" select="$RepConf"/>
				<xsl:with-param name="dT_Type" select="'99'"/>
				<xsl:with-param name="Lang" select="$vLANG"/>
				<xsl:with-param name="RepDic" select="'N'"/> 
				<xsl:with-param name="dictionary" select="$vRepDic"/> 	
				<xsl:with-param name="ForcedTitle" select="'Effectiveness for function'"/>			
			</xsl:call-template>
