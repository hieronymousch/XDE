	...
	<html>
		...
		<body>
			...
			<xsl:call-template name="Generic_Table_DBWEB">
				<xsl:with-param name="DBWEB_Name" select="'IL_MR_PREQ'"/>
				<xsl:with-param name="UdTN" select="'PREQ'"/>
				<xsl:with-param name="Lang" select="$vLANG"/>
				<xsl:with-param name="RepDic" select="'Y'"/> 
				<xsl:with-param name="dictionary" select="$vRepDic"/> 	
				<xsl:with-param name="ForcedTitle" select="'PREQ details'"/>	
				<xsl:with-param name="Col_Filtering" select="'Y'"/>	
				<xsl:with-param name="DDURL" select="'IL_MR_PREQ_DD.xml?Mode=Drill-Down&amp;pPREQ='"/>
